...
Vérifie la cohérence d'un emploie du temps sous format xlsx en le comparant à la maquette sous format xlsx.

USAGE
-----
    python xlsx_reader.py [OPTIONS]

OPTIONS
-------
    --config       Fichier de configuration TOML
                   Défaut : xls2ade/src/xlsx_reader_config.toml

    --edt          Fichier emploi du temps source (xlsx)
                   Défaut : xls2ade/xls/src/edt1sn-2024.xlsx

    --page-debut   Numéro de la première page à lire (entier)
                   Défaut : 4

    --maquette     Fichier maquette de référence (xlsx)
                   Défaut : xls2ade/xls/src/edt1sn-2024-maquette.xlsx

    --instructor   Fichier de la liste des instructeurs (csv)
                   Défaut : xls2ade/src/instructor.csv

    --alias        Fichier des alias par défaut des instructeurs (csv)
                   Défaut : xls2ade/src/instructor-default-aliases.csv

    --odf          Fichier ODF source (xlsx)
                   Défaut : xls2ade/xls/src/odf1sn-2024.xlsx

    --csv          Fichier CSV de sortie généré
                   Défaut : cours1sn.csv

    --dependances  Fichier texte des dépendances entre cours
                   Défaut : dependances1sn.txt

    --log-level    Niveau de verbosité des logs
                   Défaut : WARNING

EXEMPLES
--------
    # Exécution avec tous les paramètres par défaut
    python xlsx_reader.py

    # Spécifier un fichier EDT différent et activer les logs de debug
    python xlsx_reader.py --edt mon_edt.xlsx --log-level DEBUG

    # Changer le fichier de sortie et la page de début
    python xlsx_reader.py --csv resultats.csv --page-debut 2

    # Utiliser des fichiers dans un autre répertoire
    python xlsx_reader.py --config src/mon_config.toml --edt xls/src/edt.xlsx  --maquette xls/src/maquette.xlsx

NOTES
-----
    - Le lecteur ODF utilisé est fixé à `lecteurOdf_mcc_reader` (non configurable en CLI).
    - Tous les arguments sont optionnels grâce aux valeurs par défaut.
    - L'aide complète est disponible via : python xlsx_reader.py --help
"""


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Lecteur XLSX vers ADE")

    parser.add_argument("--config",       default="xls2ade/src/xlsx_reader_config.toml",          help="Fichier de configuration TOML")
    parser.add_argument("--edt",          default="xls2ade/xls/src/edt1sn-2024.xlsx",             help="Fichier EDT (xlsx)")
    parser.add_argument("--page-debut",   default=4, type=int,                                     help="Page de début")
    parser.add_argument("--maquette",     default="xls2ade/xls/src/edt1sn-2024-maquette.xlsx",    help="Fichier maquette (xlsx)")
    parser.add_argument("--instructor",   default="xls2ade/src/instructor.csv",                    help="Fichier instructeurs (csv)")
    parser.add_argument("--alias",        default="xls2ade/src/instructor-default-aliases.csv",    help="Fichier alias instructeurs (csv)")
    parser.add_argument("--odf",          default="xls2ade/xls/src/odf1sn-2024.xlsx",             help="Fichier ODF (xlsx)")
    parser.add_argument("--csv",          default="cours1sn.csv",                                  help="Fichier CSV de sortie")
    parser.add_argument("--dependances",  default="dependances1sn.txt",                            help="Fichier de dépendances")
    ...
    lecteurXlsx(
        ...
        args.alias,
        args.odf,
        lecteurOdf_mcc_reader,   # fonction fixe, pas un argument
        args.csv,
        args.dependances,
        logLevel=log_level,
    )